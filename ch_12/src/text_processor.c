...
text_processor_t *alloc_texts(size_t n, size_t max_len,
  ...
  for (size_t i = 0; i < n; ++i) {
    list_add_tail(&(ALLOC_INIT_BLOB(char, max_len + 1, texts[i]))->list,
                  &tp->head);
  }
  return tp;
}
...
int split_a_blob(list_head *h, int pos) {
  ...
  blob_t *split_b = ALLOC_BLOB(char, b->max_len);
  memcpy(split_b->value, b->value + pos, strlen(b->value) - pos);
  split_b->value[strlen(b->value) - pos] = '\0';
  b->value[pos] = '\0';
