...
void inverse(Matrix *in, Matrix *out) {
  ...
  for (int k = 0; k < n; ++k) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      in->div_fn(mat_ak_i, mat_ak_i, pivot);
      in->div_fn(dest_ak_i, dest_ak_i, pivot);
    }
    for (int i = 0; i < n; ++i) {
      if (i != ak) {
        void *mat_i_k =
        ...
        for (int j = 0; j < n; ++j) {
          ...
          in->mul_fn(tmp2, mat_ak_j, tmp);
          in->minus_fn(mat_i_j, mat_i_j, tmp2);
          in->mul_fn(tmp2, dest_ak_j, tmp);
          out->minus_fn(dest_i_j, dest_i_j, tmp2);
        }
      }
    }
  }
