...
typedef complex double (*Newton_method_t)(complex double, complex double,
                                          size_t);
...
static inline double du_dx_newton(Newton_method_t func, double x, double y,
                                  complex double x_0, size_t m, double h) {
  return (creal(func((x + h) + y * I, x_0, m)) -
          creal(func((x - h) + y * I, x_0, m))) /
         (2 * h);
}

static inline double du_dy_newton(Newton_method_t func, double x, double y,
                                  complex double x_0, size_t m, double h) {
  return (creal(func(x + (y + h) * I, x_0, m)) -
          creal(func(x + (y - h) * I, x_0, m))) /
         (2 * h);
}

static inline double dv_dx_newton(Newton_method_t func, double x, double y,
                                  complex double x_0, size_t m, double h) {
  return (cimag(func((x + h) + y * I, x_0, m)) -
          cimag(func((x - h) + y * I, x_0, m))) /
         (2 * h);
}

static inline double dv_dy_newton(Newton_method_t func, double x, double y,
                                  complex double x_0, size_t m, double h) {
  return (cimag(func(x + (y + h) * I, x_0, m)) -
          cimag(func(x + (y - h) * I, x_0, m))) /
         (2 * h);
}
...
static inline complex double get_m_roots_derivatives(Newton_method_t func,
                                                     complex double z,
                                                     complex double x_0,
                                                     size_t m) {
  if (cimag(z) < 1e-9) goto CAL_DERI;
  ...
  double ux =
      du_dx_newton(func, creal(z), cimag(z), x_0, m, creal(derivatives_h));
  double uy =
      du_dy_newton(func, creal(z), cimag(z), x_0, m, creal(derivatives_h));
  double vx =
      dv_dx_newton(func, creal(z), cimag(z), x_0, m, creal(derivatives_h));
  double vy =
      dv_dy_newton(func, creal(z), cimag(z), x_0, m, creal(derivatives_h));
...
CAL_DERI:
  complex double h = derivatives_h + 1.0 * derivatives_h * (cimag(z) > 0);
  return (func(z, x_0 + h, m) - func(z, x_0 - h, m)) / (2.0 * h);
}
