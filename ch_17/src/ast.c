...
int parse_int(const wchar_t* pattern[static 1]) {
  int val = 0;
...
static int parse_char_class(const wchar_t* pattern[restrict static 1],
                            CHset cs[restrict static 1]) {
  while (**pattern && **pattern != ']') {
    wchar_t c = (wchar_t) * *pattern;
    ...
    if (*(*pattern + 1) == '-' && *(*pattern + 2) && *(*pattern + 2) != ']') {
      wchar_t lo = c;
      wchar_t hi = (wchar_t) * (*pattern + 2);
      if (lo > hi) return 0;
      for (wchar_t s = lo; s <= hi; s++) add_char(s, cs);
      (*pattern) += 3;
      continue;
    }
...
static ASTNode* parse_ast_level_1(const wchar_t* pattern[static 1]);
static ASTNode* parse_ast_level_2(const wchar_t* pattern[static 1]);
static ASTNode* parse_ast_level_3(const wchar_t* pattern[restrict static 1],
                                  CHset cs[static 1]);
static ASTNode* parse_ast_level_4(const wchar_t* pattern[static 1]);
...
static ASTNode* parse_ast_level_3(const wchar_t* pattern[static 1],
                                  CHset cs[static 1]) {
...
ASTNode* complie(const wchar_t* pattern[static 1]) {
...
int match(ASTNode node[restrict static 1], const wchar_t* restrict text,
          Cont cont, void* restrict data);
static int match_star(ASTNode child[static 1], const wchar_t* restrict text,
                      Cont cont, void* restrict data);

static int concat_cont(const wchar_t* restrict text, void* restrict data) {
  ConcatCtx* c = (ConcatCtx*)data;
  return match(c->right, text, c->cont, c->data);
}

static int star_inner_cont(const wchar_t* restrict text, void* restrict data) {
...
static int match_braces(ASTNode child[static 1], int lo, int hi, int cur,
                        const wchar_t* restrict text, Cont cont,
                        void* restrict data);

static int braces_inner_cont(const wchar_t* restrict text,
                             void* restrict data) {
...
static int final_cont(const wchar_t* restrict text, void* restrict data) {
...
const wchar_t* regex_search(ASTNode root[restrict static 1],
                            const wchar_t* restrict text,
                            const wchar_t* match_end[restrict static 1]) {
