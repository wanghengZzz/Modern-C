...
inline static void add_char(wchar_t c, CHset cs[static 1]) {
  cs->ch_set[c >> 3] |= (1u << (c & 0x07));
}

inline static int is_in_charset(wchar_t c, CHset cs[static 1]) {
  return cs->ch_set[c >> 3] & (1u << (c & 0x07));
}

inline static int check_char_range(wchar_t l, wchar_t r, wchar_t c) {
  return c >= l && c <= r;
}
...
#define INIT_CHset(str, cs)                                                   \
  ({                                                                          \
    for (size_t __i = 0; __i < wcslen(str); ++__i) add_char((str)[__i], &cs); \
  })

void dump_ast(ASTNode node[static 1], size_t level);

typedef int (*Cont)(const wchar_t *text, void *data)
